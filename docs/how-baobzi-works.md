...
Baobzi turns an expensive function `f(x)` into a cheap lookup by precomputing
a **piecewise Chebyshev approximation** on an **adaptive tree**. Cost: memory
...
```
   user calls fn(x), fn(x), fn(x)... a million times.
              ...
              ▼
   ┌──────────────────────────────────────┐
   │  fit once:  baobzi::fit(f, a, b)     │  ◄── slow: runs f(x) on Chebyshev
...
Start with the whole domain as one box. Try to fit a degree-D Chebyshev
polynomial. If error too large, split into `2^Dim` children and recurse.
...
```
   ...
   polyfits_[]   — per-leaf Chebyshev coefficients (packed)
   ┌──────────┬──────────┬──────────┬──────────┐
   │   P0     │   P1     │   ...    │   Pk     │
   └──────────┴──────────┴──────────┴──────────┘
```
...
The leaf carries Chebyshev coefficients `c[k_0, k_1, ..., k_{n-1}]`.
...
This is what makes baobzi fast on million-point batches. Naive batch:
...
```
   baobzi  =  k-d tree of Chebyshev polynomials
