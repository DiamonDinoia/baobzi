#ifndef BAOBZI_EVAL_SCATTER_HPP
#define BAOBZI_EVAL_SCATTER_HPP

/// \file baobzi/eval_scatter.hpp
...
/// `operator()` (BAOBZI_ALWAYS_INLINE — no per-pair callq).
///
...
auto eval_scatter_sorted(
    std::span<const Function<Degree, Func> *const> fits,
    std::span<const std::uint32_t> fit_ids,
    std::span<const double> xs,
    std::span<double> ys,
    std::uint32_t n_fits,
    std::span<std::uint32_t> counts,
    std::span<std::uint32_t> perm) -> void
{
    using fn_t = Function<Degree, Func>;
    static_assert(fn_t::input_dim == 1 && fn_t::output_dim == 1,
                  "eval_scatter_sorted: 1D scalar fits only");
    ...
    std::exclusive_scan(counts.begin(), counts.begin() + n_fits,
                        counts.begin(), std::uint32_t{0});
    ...
    // Dispatch each non-empty fit-id run. fn(x) is BAOBZI_ALWAYS_INLINE,
...
auto eval_scatter_sorted(
    ...
    std::span<const double> xs,
    std::span<double> ys,
    std::uint32_t n_fits) -> void
{
    std::vector<std::uint32_t> counts(n_fits);
    std::vector<std::uint32_t> perm(fit_ids.size());
    eval_scatter_sorted<Degree, Func>(fits, fit_ids, xs, ys, n_fits,
                                       std::span<std::uint32_t>(counts),
