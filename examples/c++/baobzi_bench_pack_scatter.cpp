...
//   * baobzi::eval_scatter_sorted(...) — multi-Function scattered eval;
...
//   taskset -c 2 ./baobzi_bench_pack_scatter > /tmp/bench_after.txt
//
...
// --- 1D kernels (mirrors baobzi_microbench's set; kept small so this
//     file stays focused on the batch-surface measurements). ---
auto make_runge1d()     { return [](double x) { return 1.0 / (1.0 + 25.0 * x * x); }; }
auto make_tanh_sharp1d(){ return [](double x) { return std::tanh(50.0 * x); }; }
...
auto make_tanh500()     { return [](double x) { return std::tanh(500.0 * x); }; }
auto make_tanh1000()    { return [](double x) { return std::tanh(1000.0 * x); }; }

auto make_bench() {
    ankerl::nanobench::Bench b;
    b.title("baobzi pack/scatter")
     .unit("eval")
     .relative(true)
     .warmup(3)
...
void bench_pack(ankerl::nanobench::Bench &b, const char *label,
                const FitT &fn, double lo, double hi,
                std::size_t n_packs) {
    ...
    for (auto &p : packs)
        for (auto &x : p) x = d(gen);

    const std::size_t total_evals = n_packs * N;

    // (1) eval_pack<N> path.
    {
        std::string name = std::string(label) + " eval_pack<" +
    ...
    {
        std::string name = std::string(label) + " scalar_loop<" +
                           std::to_string(N) + "> deg=" + std::to_string(Deg);
        b.batch(static_cast<double>(total_evals));
        b.run(name, [&] {
            double acc = 0.0;
...
void sweep_pack_1d(ankerl::nanobench::Bench &b, const char *label,
                   FmakeT make_f, double lo, double hi,
                   double tol = 1e-10) {
    auto fn = baobzi::fit<Deg>(make_f(), lo, hi, tol);
...
void sweep_scatter_1d(ankerl::nanobench::Bench &b, const char *label,
                      std::size_t R) {
    ...
    // Build R distinct smooth fits over [0, 1] — small random Chebyshev
    ...
    using fit_t = decltype(baobzi::fit<Deg>(std::declval<ff &>(),
                                            0.0, 1.0, 1e-10));
    std::vector<fit_t> fits;
    fits.reserve(R);
    for (auto &f : exact) fits.push_back(baobzi::fit<Deg>(f, 0.0, 1.0, 1e-10));
    ...
    for (std::size_t n_pts : {std::size_t(64), std::size_t(256),
                              std::size_t(1024)}) {
        ...
        const std::string suffix = std::string(" deg=") + std::to_string(Deg) +
                                   " R=" + std::to_string(R) +
        ...
        // (1) eval_scatter_sorted path — counting-sort overload.
        {
            ...
            b.run(name, [&] {
                baobzi::eval_scatter_sorted<Deg, ff>(
                    std::span<const fit_t *const>{fit_ptrs.data(), fit_ptrs.size()},
                    std::span<const std::uint32_t>{ids.data(), ids.size()},
                    std::span<const double>{xs.data(), xs.size()},
                    std::span<double>{ys.data(), ys.size()},
                    static_cast<std::uint32_t>(R));
                ankerl::nanobench::doNotOptimizeAway(ys.data());
            });
        }
...
int main() {
    std::printf("# baobzi pack/scatter bench (nanobench, xsimd lane_w=%zu)\n",
    ...
    // exercises the inlining of polyfit's evalCanonical into baobzi.
    // Probe asm showed 1 callq/eval in 2D and 2 in 3D before
    // BAOBZI_FLATTEN on operator(); 1 callq stays in both after.
    {
        ...
        auto fn2d = baobzi::fit<8>(bump2d, std::array{0.0, 0.0},
                                   std::array{1.0, 1.0}, /*tol=*/1e-10);
        auto fn3d = baobzi::fit<8>(gauss3d, std::array{-1.0, -1.0, -1.0},
        ...
        b.run("2d_bump scalar_loop<256> deg=8", [&] {
            double acc = 0.0;
            for (std::size_t i = 0; i < kN; ++i) acc += fn2d(xs2[i])[0];
            ankerl::nanobench::doNotOptimizeAway(acc);
        });
        b.run("3d_gauss scalar_loop<256> deg=8", [&] {
            double acc = 0.0;
            for (std::size_t i = 0; i < kN; ++i) acc += fn3d(xs3[i])[0];
            ankerl::nanobench::doNotOptimizeAway(acc);
        });
    }
