...
template <class X> struct dim_of;
template <class T, std::size_t N> struct dim_of<std::array<T, N>> {
    static constexpr std::size_t value = N;
};
template <> struct dim_of<double> { static constexpr std::size_t value = 1; };

template <class Fn, class X>
void time_and_check(const std::string &label, Fn &fn, const std::vector<X> &xs,
                    auto exact) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        if constexpr (in_dim == 1)
            ...
            for (std::size_t d = 0; d < in_dim; ++d) flat[in_dim * i + d] = xs[i][d];
    }
    ...
    constexpr std::size_t out_dim = []() -> std::size_t {
        if constexpr (requires { std::declval<Fn>()(std::declval<X>())[0]; })
            return std::tuple_size_v<
                std::remove_cvref_t<decltype(std::declval<Fn>()(std::declval<X>()))>>;
        else
            return 1;
    }();
    ...
    std::cout << "[" << label << "] MEvals/s = "
              << static_cast<double>(n) / (dt * 1e6)
              << "  max_rel_err = " << mx_rel
              << "  max_abs_err = " << mx_abs
              << "  (ignore " << acc << ")\n";
}
...
void run_1d(std::size_t n_points) {
    ...
    auto fn = baobzi::fit<10>(f, -1.0, 1.0, /*tol=*/1e-10);
...
void run_2d(std::size_t n_points) {
    ...
    auto fn = baobzi::fit<10>(f, std::array{0.0, 0.0},
...
void run_3d(std::size_t n_points) {
    ...
    auto fn = baobzi::fit<8>(f, std::array{-1.0, -1.0, -1.0},
...
void run_yukawa_3d(std::size_t n_points) {
    ...
    auto fn = baobzi::fit<8>(f, std::array{0.2, 0.2, 0.2},
                             std::array{1.5, 1.5, 1.5}, /*tol=*/1e-8);
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2) n_points = static_cast<std::size_t>(std::atoll(argv[1]));
    if (argc > 2) {
        dims.clear();
        for (int i = 2; i < argc; ++i) dims.push_back(std::atoi(argv[i]));
    }

    for (int d : dims) {
        switch (d) {
            case 1: run_1d(n_points); break;
            case 2: run_2d(n_points); break;
            case 3: run_3d(n_points); break;
            case 4: run_yukawa_3d(n_points); break;
            default:
                std::cerr << "Only 1, 2, 3, 4(=yukawa3d) supported; got "
                          << d << "\n";
                return 1;
        }
    }
