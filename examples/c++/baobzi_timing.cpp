...
void time_and_check(const std::string &label, Fn &fn, const std::vector<X> &xs,
    ...
    std::cout << "[" << label << "] MEvals/s = "
              << static_cast<double>(xs.size()) / (dt * 1e6)
              << "  max_rel_err = " << mx_rel
              << "  max_abs_err = " << mx_abs
              << "  (ignore " << acc << ")\n";
}
...
void run_1d(std::size_t n_points) {
    ...
    auto fn = baobzi::fit<10>(f, -1.0, 1.0);
...
int main(int argc, char *argv[]) {
    ...
    for (int d : dims) {
        switch (d) {
            ...
            default:
                std::cerr << "Only 1, 2, 3D supported; got " << d << "\n";
                return 1;
        }
    }
    return 0;
