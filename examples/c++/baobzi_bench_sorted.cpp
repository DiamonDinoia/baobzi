...
// `operator()(xp,res,n)` is covered by baobzi_microbench / pack_scatter).
...
//
//   taskset -c 2 ./baobzi_bench_sorted > /tmp/sorted_after.txt
...
void warm_core() {
    ...
    while (std::chrono::steady_clock::now() - t0 < std::chrono::milliseconds(300))
        for (int i = 0; i < 100000; ++i) acc += std::sqrt(static_cast<double>(i) + 1.0);
    ankerl::nanobench::doNotOptimizeAway(acc);
}

template <std::size_t Deg, class FmakeT>
void sweep_sorted_1d(ankerl::nanobench::Bench &b, const char *label,
                     FmakeT make_f, double lo, double hi, double tol = 1e-10) {
    ...
    for (std::size_t n : {std::size_t(32), std::size_t(1024), std::size_t(1'000'000)}) {
        ...
        if (n >= 1'000'000) b.minEpochTime(std::chrono::milliseconds(100));
        else                b.minEpochTime(std::chrono::milliseconds(300));

        std::string name = std::string(label) + " sorted deg=" +
                           std::to_string(Deg) + " N=" + std::to_string(n);
        b.batch(static_cast<double>(n));
        b.run(name, [&] {
            fn.sorted(xs.data(), res.data(), n);
            ankerl::nanobench::doNotOptimizeAway(res.data());
        });
    }
}
...
int main() {
    std::printf("# baobzi sorted bench (nanobench, xsimd lane_w=%zu)\n",
