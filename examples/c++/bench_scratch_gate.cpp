...
template <class Fn>
void sweep_1d(ankerl::nanobench::Bench &b, const Fn &fn,
              double a, double b_) {
    std::mt19937 gen(7);
...
template <class Fn>
void sweep_3d(ankerl::nanobench::Bench &b, const Fn &fn,
              std::array<double, 3> a, std::array<double, 3> b_) {
    std::mt19937 gen(7);
...
int main() {
    ...
    b.title("baobzi batch (AoS, thread_local scratch)")
     .unit("eval")
     .relative(true)
     .warmup(3)
    ...
    auto fn1d = baobzi::fit<8>(make_runge1d(), -1.0, 1.0, 1e-10, opts);
    auto fn3d = baobzi::fit<8>(make_gauss3d(),
