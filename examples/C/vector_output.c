 ...
 * two batch layouts: AoS (`baobzi_eval_multi_f64`, results packed
 * [c0,c1,c2, c0,c1,c2, ...]) and SoA (`baobzi_eval_multi_soa_f64`, one
...
int main(void) {
    ...
    baobzi_t fn = baobzi_fit_f64(f, NULL,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            if (d > max_soa_aos_diff) max_soa_aos_diff = d;
    ...
    fn = baobzi_free(fn);
    return (max_soa_aos_diff == 0.0 && max_abs_err < 1e-6) ? EXIT_SUCCESS
                                                           : EXIT_FAILURE;
}
