...
TEST_CASE("batch path routes through caller-supplied allocator",
          "[baobzi][batch][allocator]") {
    auto fn = baobzi::fit<8>([](double x) { return std::sin(4.0 * x); },
                             0.0, 1.0, 1e-10);
    ...
    for (std::size_t i = 0; i < N; ++i) REQUIRE(out[i] == ref[i]);
}

TEST_CASE("default-allocator batch path matches custom-allocator path",
          "[baobzi][batch][allocator]") {
    // Vector output: exercises the SoA overload too.
    auto fn = baobzi::fit<8>(
    ...
    std::array<double *, 2> soa{c0.data(), c1.data()};
