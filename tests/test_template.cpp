...
TEST_CASE("1D1 evaluations", "[baobzi_template]") {
    ...
    auto baobzifunc = baobzi::fit<8>(testfun_1d1, a, b, tol);
...
TEST_CASE("1D array->array evaluations", "[baobzi_template]") {
    auto f = [](std::array<double, 1> x) -> std::array<double, 4> {
        const double v = x[0];
        return {std::sin(v), std::cos(v), std::exp(-v * v),
                1.0 / (1.0 + v * v)};
    };
    ...
    auto fn = baobzi::fit<8>(f, a, b, tol);
    ...
    SECTION("unsorted and sorted batch overloads match per-point eval") {
        ...
        for (std::size_t i = 0; i < N; ++i)
            xs[i] = lo + (hi - lo) * (static_cast<double>(i) + 0.5) /
                            static_cast<double>(N);

        std::vector<double> ref(4 * N);
        for (std::size_t i = 0; i < N; ++i) {
            const auto y = fn(std::array<double, 1>{xs[i]});
            for (std::size_t k = 0; k < 4; ++k) ref[i * 4 + k] = y[k];
        }
