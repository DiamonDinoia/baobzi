...
std::vector<double> make_inputs(std::array<double, Dim> a,
...
std::vector<double> run_chunked(Fn &fn, const std::vector<double> &xp,
                                std::size_t n) {
...
template <std::size_t Dim, class Fn>
void run_threadsafe_check(Fn &fn,
                          const std::vector<double> &xp,
                          const std::vector<double> &res_ref) {
    ...
    for (int rep = 1; rep < kRepeats; ++rep) {
        ...
        REQUIRE(std::memcmp(res.data(), res0.data(),
                            n * sizeof(double)) == 0);
    }
