...
#   BAOBZI_BUILD  build dir holding libbaobzi_c.{so,a}  (default: ../../build)
#   PYTHON        python interpreter with `baobzi` installed (default: python3)
...
repo="$(cd "$here/../.." && pwd)"
build="${BAOBZI_BUILD:-$repo/build}"
PYTHON="${PYTHON:-python3}"
JULIA="${JULIA:-julia}"
TOL="${TOL:-1e-9}"

lib_so="$build/libbaobzi_c.so"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT

# --- C reference (mandatory) ---------------------------------------------
echo "== C reference =="
cc "$here/reference.c" -I"$repo/include" -L"$build" -lbaobzi_c -lm \
   -Wl,-rpath,"$build" -o "$tmp/reference" || { echo "FATAL: cannot build reference.c"; exit 2; }
"$tmp/reference" > "$tmp/ref.csv" || { echo "FATAL: reference run failed"; exit 2; }
cat "$tmp/ref.csv"
...
# --- Python --------------------------------------------------------------
echo "== Python =="
if "$PYTHON" -c "import baobzi" 2>/dev/null; then
    if "$PYTHON" "$here/parity.py" > "$tmp/py.csv" 2>"$tmp/py.err"; then
        compare Python "$tmp/py.csv" || status=1
    else
        echo "FAIL Python: parity.py errored"; cat "$tmp/py.err"; status=1
    fi
else
    echo "SKIP Python: 'baobzi' not importable in $PYTHON (pip install ./bindings/python)"
fi

# --- Julia ---------------------------------------------------------------
echo "== Julia =="
if command -v "$JULIA" >/dev/null 2>&1; then
    if LIBBAOBZI_C="$lib_so" "$JULIA" --project="$repo/bindings/julia/Baobzi" \
            "$here/parity.jl" > "$tmp/jl.csv" 2>"$tmp/jl.err"; then
        compare Julia "$tmp/jl.csv" || status=1
    else
        echo "FAIL Julia: parity.jl errored"; cat "$tmp/jl.err"; status=1
    fi
else
