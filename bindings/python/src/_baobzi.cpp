/* _baobzi.cpp — nanobind bindings for the baobzi C ABI.
 *
 * Architecture:
 ...
 *   - After baobzi_fit_* returns we re-raise the stashed exception (via
 *     nb::raise_python_error) so the original Python exception propagates
 *     intact to the caller.
 *   - BaobziFunction wraps a baobzi_t handle and exposes eval / eval_multi /
 *     sorted / eval_multi_soa methods plus introspection properties. The C++
 *     destructor calls baobzi_free, so Python GC suffices for cleanup.
 */
...
// ---------------------------------------------------------------------------
// Trampoline state (one per baobzi_fit_* call)
// ---------------------------------------------------------------------------
...
static void trampoline_f64(const double *x, double *y, void *data) {
    ...
    if (st->errored) {
        for (int i = 0; i < st->output_dim; ++i) y[i] = kNaN64;
        return;
    }
    ...
    try {
        ...
        nb::ndarray<const double, nb::numpy, nb::shape<-1>,
                    nb::c_contig, nb::device::cpu>
            xarr(const_cast<double *>(x), {(size_t)st->input_dim});
        ...
        if (st->output_dim == 1) {
            ...
            nb::ndarray<const double, nb::numpy, nb::shape<-1>,
                        nb::c_contig, nb::device::cpu>
                rarr = nb::cast<nb::ndarray<const double, nb::numpy,
                                            nb::shape<-1>, nb::c_contig,
                                            nb::device::cpu>>(result);
            ...
            for (int i = 0; i < st->output_dim; ++i) y[i] = rp[i];
        }
...
static void trampoline_f32(const float *x, float *y, void *data) {
    ...
    if (st->errored) {
        for (int i = 0; i < st->output_dim; ++i) y[i] = kNaN32;
        return;
    }
    ...
    try {
        nb::ndarray<const float, nb::numpy, nb::shape<-1>,
                    nb::c_contig, nb::device::cpu>
            xarr(const_cast<float *>(x), {(size_t)st->input_dim});
        ...
        if (st->output_dim == 1) {
            ...
            nb::ndarray<const float, nb::numpy, nb::shape<-1>,
                        nb::c_contig, nb::device::cpu>
                rarr = nb::cast<nb::ndarray<const float, nb::numpy,
                                            nb::shape<-1>, nb::c_contig,
                                            nb::device::cpu>>(result);
            if ((int)rarr.shape(0) != st->output_dim)
                throw std::runtime_error("callback returned wrong number of outputs");
...
// ---------------------------------------------------------------------------
// BaobziFunction — bound C++ class holding a baobzi_t handle.
// ---------------------------------------------------------------------------
class BaobziFunction {
public:
    // Takes ownership of the handle; destructor calls baobzi_free.
    explicit BaobziFunction(baobzi_t h) : handle_(h) {}

    ~BaobziFunction() {
        if (handle_) handle_ = baobzi_free(handle_);
    }

    // Non-copyable, moveable.
    BaobziFunction(const BaobziFunction &)            = delete;
    BaobziFunction &operator=(const BaobziFunction &) = delete;

    // ---- introspection ---------------------------------------------------
    int    input_dim()     const { return baobzi_input_dim(handle_); }
    int    output_dim()    const { return baobzi_output_dim(handle_); }
    size_t memory_usage()  const { return baobzi_memory_usage(handle_); }
    int    dtype_tag()     const { return (int)baobzi_dtype(handle_); }
    std::string dtype_str() const {
        return (baobzi_dtype(handle_) == BAOBZI_F64) ? "f64" : "f32";
    }
    void   print_stats()   const { baobzi_print_stats(handle_); }

    // ---- scalar eval (1 point) ------------------------------------------
    nb::object eval_one(nb::object x_obj) const {
        const int id = input_dim();
        const int od = output_dim();
        bool is_f32 = (baobzi_dtype(handle_) == BAOBZI_F32);

        if (is_f32) {
            std::vector<float> xv(id), yv(od);
            _fill_x_f32(x_obj, xv.data(), id);
            baobzi_eval_f32(handle_, xv.data(), yv.data());
            if (od == 1) return nb::cast(yv[0]);
            return _make_f32_array(yv.data(), od);
        } else {
            std::vector<double> xv(id), yv(od);
            _fill_x_f64(x_obj, xv.data(), id);
            baobzi_eval_f64(handle_, xv.data(), yv.data());
            if (od == 1) return nb::cast(yv[0]);
    ...
    nb::object eval_multi_py(nb::object x_obj) const {
        ...
        if (is_f32) {
            ...
            baobzi_eval_multi_f32(handle_, x_arr.data(), res.data(), n);
            return _make_output_f32(res.data(), n, od);
        } else {
            auto x_arr = _coerce_input_f64(x_obj, id);
            size_t n = x_arr.shape(0);
            std::vector<double> res(n * od);
            baobzi_eval_multi_f64(handle_, x_arr.data(), res.data(), n);
    ...
    nb::object eval_sorted_py(nb::object x_obj) const {
        ...
        if (is_f32) {
            ...
            size_t n = x_arr.shape(0);
            std::vector<float> res(n * od);
            baobzi_eval_sorted_f32(handle_, x_arr.data(), res.data(), n);
            return _make_output_f32(res.data(), n, od);
        } else {
            auto x_arr = _coerce_input_f64(x_obj, 1);
            size_t n = x_arr.shape(0);
            std::vector<double> res(n * od);
            baobzi_eval_sorted_f64(handle_, x_arr.data(), res.data(), n);
            return _make_output_f64(res.data(), n, od);
        }
    }
    ...
    nb::object eval_multi_soa_py(nb::object x_obj) const {
        ...
        if (is_f32) {
            ...
            for (int d = 0; d < od; ++d) soa[d] = comps[d].data();
            baobzi_eval_multi_soa_f32(handle_, x_arr.data(), soa.data(), n);
            nb::list out;
            for (int d = 0; d < od; ++d)
            ...
            for (int d = 0; d < od; ++d) soa[d] = comps[d].data();
            baobzi_eval_multi_soa_f64(handle_, x_arr.data(), soa.data(), n);
            nb::list out;
            for (int d = 0; d < od; ++d)
...
private:
    ...
    using F64Array = nb::ndarray<const double, nb::numpy, nb::c_contig,
                                 nb::device::cpu>;
    using F32Array = nb::ndarray<const float, nb::numpy, nb::c_contig,
                                 nb::device::cpu>;
    // Mutable views, used when writing into a freshly-allocated output array.
    using F64ArrayMut = nb::ndarray<double, nb::numpy, nb::c_contig,
                                    nb::device::cpu>;
    using F32ArrayMut = nb::ndarray<float, nb::numpy, nb::c_contig,
                                    nb::device::cpu>;

    static F64Array _coerce_input_f64(nb::object x_obj, int id) {
        // Accept any array-like; ndim may be 1 (for id==1) or 2 (for id>1).
        return nb::cast<F64Array>(
    ...
    static void _fill_x_f64(nb::object x, double *out, int id) {
        if (id == 1) {
            ...
            auto arr = nb::cast<F64Array>(
                nb::module_::import_("numpy").attr("asarray")(
                    x, nb::module_::import_("numpy").attr("float64")));
            for (int i = 0; i < id; ++i) out[i] = arr.data()[i];
        }
    }
    static void _fill_x_f32(nb::object x, float *out, int id) {
        if (id == 1) {
            ...
            auto arr = nb::cast<F32Array>(
                nb::module_::import_("numpy").attr("asarray")(
                    x, nb::module_::import_("numpy").attr("float32")));
            for (int i = 0; i < id; ++i) out[i] = arr.data()[i];
        }
    }
...
// ---------------------------------------------------------------------------
// fit_f64 / fit_f32 — called from Python's baobzi.fit()
// ---------------------------------------------------------------------------

static nb::object fit_f64(
    nb::object callable,
    ...
    int allow_max_depth_leaves, int min_uniform_depth)
{
    ...
    opts.tol_kind              = (baobzi_tol_kind_t)tol_kind_int;
    ...
    baobzi_t handle = baobzi_fit_f64(
        trampoline_f64, &st,
        input_dim, output_dim, degree,
        a.data(), b.data(), tol,
        (baobzi_eval_policy_t)policy_int,
        &opts);

    // Handle callback exception first (takes priority).
    if (st.errored) {
        if (handle) handle = baobzi_free(handle); // shouldn't happen but be safe
        if (st.exc) raise_stashed(st.exc);
        throw std::runtime_error("baobzi callback failed (no exception stashed)");
    }

    if (!handle) {
        const char *msg = baobzi_last_error();
        throw std::runtime_error(msg && *msg ? msg : "baobzi_fit_f64 returned NULL");
    }

    return nb::cast(new BaobziFunction(handle),
                    nb::rv_policy::take_ownership);
}

static nb::object fit_f32(
    nb::object callable,
    int input_dim, int output_dim, int degree,
    ...
    int allow_max_depth_leaves, int min_uniform_depth)
{
    ...
    baobzi_t handle = baobzi_fit_f32(
        trampoline_f32, &st,
        input_dim, output_dim, degree,
        af.data(), bf.data(), tol,
        (baobzi_eval_policy_t)policy_int,
    ...
    if (st.errored) {
        if (handle) handle = baobzi_free(handle);
        if (st.exc) raise_stashed(st.exc);
        throw std::runtime_error("baobzi callback failed (no exception stashed)");
    }
    ...
    if (!handle) {
        const char *msg = baobzi_last_error();
        throw std::runtime_error(msg && *msg ? msg : "baobzi_fit_f32 returned NULL");
    }
...
// ---------------------------------------------------------------------------
NB_MODULE(_baobzi, m) {
    m.doc() = "Low-level nanobind bindings for the baobzi C ABI.";

    // Enum mirrors
    nb::enum_<baobzi_tol_kind_t>(m, "TolKind")
        .value("RELATIVE_TAIL", BAOBZI_RELATIVE_TAIL)
        .value("ABSOLUTE_TAIL", BAOBZI_ABSOLUTE_TAIL)
        .value("RELATIVE_MAX",  BAOBZI_RELATIVE_MAX)
        .value("ABSOLUTE_MAX",  BAOBZI_ABSOLUTE_MAX)
        .value("RELATIVE_L2",   BAOBZI_RELATIVE_L2)
        .value("ABSOLUTE_L2",   BAOBZI_ABSOLUTE_L2);

    nb::enum_<baobzi_eval_policy_t>(m, "EvalPolicy")
        .value("LATENCY",    BAOBZI_LATENCY)
        .value("THROUGHPUT", BAOBZI_THROUGHPUT)
        .value("BALANCED",   BAOBZI_BALANCED);

    nb::enum_<baobzi_dtype_t>(m, "DType")
        .value("F64", BAOBZI_F64)
        .value("F32", BAOBZI_F32);

    nb::class_<BaobziFunction>(m, "BaobziFunction",
        "Opaque handle to a fitted baobzi function. Do not construct directly;"
        " use _baobzi.fit_f64() or _baobzi.fit_f32().")
        .def("eval_one",        &BaobziFunction::eval_one,
             "Evaluate at a single point (scalar or length-dim sequence).",
             "x"_a)
        .def("eval_multi",      &BaobziFunction::eval_multi_py,
             "AoS batch eval. x: (N, dim) or (N,) for 1D.", "x"_a)
        .def("sorted",          &BaobziFunction::eval_sorted_py,
             "Sorted-1D batch eval (input_dim==1).", "x"_a)
        .def("eval_multi_soa",  &BaobziFunction::eval_multi_soa_py,
             "SoA batch eval. Returns list of (N,) arrays, one per output component.",
             "x"_a)
        .def("print_stats",     &BaobziFunction::print_stats)
        .def_prop_ro("input_dim",    &BaobziFunction::input_dim)
        .def_prop_ro("output_dim",   &BaobziFunction::output_dim)
        .def_prop_ro("memory_usage", &BaobziFunction::memory_usage)
        .def_prop_ro("dtype",        &BaobziFunction::dtype_str);

    m.def("fit_f64", &fit_f64,
          "callable"_a, "input_dim"_a, "output_dim"_a, "degree"_a,
          "a"_a, "b"_a, "tol"_a,
          "policy"_a, "tol_kind"_a,
          "max_depth"_a, "max_memory_mib"_a,
          "allow_max_depth_leaves"_a, "min_uniform_depth"_a);

    m.def("fit_f32", &fit_f32,
          "callable"_a, "input_dim"_a, "output_dim"_a, "degree"_a,
          "a"_a, "b"_a, "tol"_a,
          "policy"_a, "tol_kind"_a,
          "max_depth"_a, "max_memory_mib"_a,
